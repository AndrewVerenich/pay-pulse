...
- Related plan section: [`docs/pay-pulse-platform-plan.md`](../pay-pulse-platform-plan.md) § observability (tracing marked optional), stage S8
...
Tracing в плане помечен как **optional**, не acceptance gate S8/S10.
...
- S8 acceptance фокусируется на metrics + dashboards + alerts, которые уже есть в коде.
...
1. **Full OTel + Tempo now** — deferred: стоимость > ценности для demo SLO; plan already optional.
...
4. **Log-only correlation IDs without metrics** — rejected: S8 требует Prometheus/Grafana explicitly.
...
- Platform plan § «Distributed tracing (опционально)».

**Revisit triggers**

- Production p99 SLO требует waterfall across gateway → saga → participants.
- Первый инкремент: W3C `traceparent` в gateway filter + copy в Kafka headers; Flink / consumers — вторым этапом.
